...
void CameraPlugin::OnUpdate()
{
    ...
    for(size_t i = 0; i < proc_subs.size(); i++)
    {
		...
		cv::Point imagePoint1 = imagePts[2];
		int imageX = imagePoint1.x;
		int imageY = imagePoint1.y;

		int originX = 320;
		int originY = 400;

		double tangValue = double(imageX - originX) / double(originY - imageY);
		double angle = atan(tangValue);
...
std::vector<cv::Point> CameraPlugin::vanishPoint(Mat mat, int mid)
{
    vector<Vec2f> lines;
